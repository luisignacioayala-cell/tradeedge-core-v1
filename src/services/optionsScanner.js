// Minimal demo unusual options scanner + basic exposure stats (fake math in demo)
...
function genAlert(symbol){
  ...
  const expiries = [7,14,21,30,45,60].map(d=>{
    const dt = new Date(now); dt.setDate(dt.getDate()+d); return dt.toISOString().slice(0,10)
  })
  ...
  return {
    ...
    symbol, expiry: pick(expiries), strike, type,
    last: Number(price), volume: vol, openInterest: oi, notionalUSD: notional,
...
export async function runDemoScan(symbols){
  ...
  state.addScanJob({ id: jobId, startedAt: new Date().toISOString(), status: 'RUNNING', symbols, found: 0 })
  ...
  state.updateScanJob(jobId, { finishedAt: new Date().toISOString(), status: 'SUCCESS', found: alerts.length })
  return alerts.length
}

export function calcDemoExposure(symbol){
  // Fake but consistent numbers for GEX/DEX/PCR
  let seed = 0
